...
export async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const availableProfit = await getAvailableProfitTwd(db);
    const recentLedger = await db.select().from(ledgerEntries).orderBy(desc(ledgerEntries.createdAt)).limit(10);
    return ok(res, { totals: { twd: twd.value, rmb: rmb.value, receivable: receivable.value, inventory: inventory.value, profit: availableProfit.toFixed(2) }, recentLedger });
  } catch (error) {
